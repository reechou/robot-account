...
func InitDB(cfg *config.Config) {
	...
	if err = x.Sync2(new(Robot),
		...
		new(WxCircle),
		new(WxCircleSetting),
		new(RobotWxCircle)); err != nil {
		holmes.Fatal("Fail to sync database: %v", err)
	}

	for i := 0; i < ROBOT_FRIEND_TABLE_NUM; i++ {
		if err = x.Sync2(&RobotFriend{RobotId: int64(i)}); err != nil {
			holmes.Fatal("Fail to sync database: %v", err)
		}
	}

	for i := 0; i < ROBOT_CHAT_TABLE_NUM; i++ {
		if err = x.Sync2(&RobotChat{RobotId: int64(i)}); err != nil {
			holmes.Fatal("Fail to sync database: %v", err)
		}
	}
}
